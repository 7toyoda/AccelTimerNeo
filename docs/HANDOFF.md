...
最終更新時の状態: バージョン 0.1.85 系 / Swift 6 / SwiftUI / SwiftData / iOS 17+。
リポジトリ: GitHub `7toyoda/AccelTimerNeo`（main ブランチ運用）。
作業コピーは `/Users/user01/dev/AccelTimerNeo`。Claude担当の `AccelTimer`、Codex担当の `AccelTimerX` とは
Git / Xcode プロジェクトを分離して運用する。

**このセッション（〜v0.1.85）の主な変更:** ①課金を透かしモデル→トライアル課金へ全面変更
...
⑪別アプリとして実機比較するため `AccelTimerX` を一時的に同一リポジトリへ追加し、最終的に
`/Users/user01/dev/AccelTimerX` の独立プロジェクトへ分離（§13, v0.1.72〜0.1.76）。
⑫AccelTimerX限定の実験的GPSバッチ遅延対策として、処理時刻から2秒超古いGPS fixは
READY/発進などライブ状態遷移に使わず、CoreMotion由来スプリットも直近GPS補正が1.6秒以内の時だけ
許可する（§2/§13/§14, v0.1.73〜0.1.74）。通常の `AccelTimer` では無効。
⑬このリポジトリから `AccelTimerX` ターゲット/フォルダを削除し、元アプリを `AccelTimer` 単体構成へ戻した
（§13, v0.1.76）。
⑭AccelTimerNeo単体運用として、表示名変数を `AccelTimerNeo` に統一し、テストターゲットのBundle IDと
テストimportも `AccelTimerNeo` へ揃えた（v0.1.83）。
⑮ソースフォルダとローカルStoreKit設定も `AccelTimerNeo` 名へ統一し、IAP商品IDを
`com.acceltimer.app.AccelTimerNeo.unlock` のみに整理した（v0.1.84）。計測ロジックへの影響はない。
⑯テストフォルダ/参照名を `AccelTimerNeoTests` に統一し、Xcode上の見間違いを減らした（v0.1.85）。
...
- スプリット時刻（0→40/60/80/100 km/h、mph表示時は0→15/30/45/60 mph）は
  **線形補間**でミリ秒精度算出。純粋関数 `TimerEngine.interpolatedCrossTime` に抽出済み
  （`AccelTimerNeoTests` でテスト）。mphスプリットは表示用に並行記録し、完了判定は従来通り
  100 km/h到達。
...
- 発進トリガー条件: `confirmedStoppedWhileArmed && READY成立から0.5秒以上 &&
  ...
  **停車確認（GPS良好で速度≈0）が前提**。
- **GPS fix鮮度ゲート（v0.1.73〜0.1.74・AccelTimerX限定）**: `CLLocation.timestamp` が処理時刻より2秒超古い
  GPSサンプルは、ログ・履歴解析には残すが、ARMEDの停車確認/READY/発進トリガー/表示速度/Kalman補正には
  使わない。2026-06-22ログでGPSが数秒〜十数秒バッチ配信される現象を確認しており、古い停止fixが
  走行中に届いて READY 化したり、古い発進fixで遅れて START するのを防ぐ。debug.csv の event には
  `GPS_STALE_IGNORED` を出す。**元の `AccelTimer` では無効**（Bundle ID が
  `com.acceltimer.app.AccelTimerX` の時だけ `AppInfo.isAccelTimerX` で有効化）。
- **Motion split のGPS鮮度ゲート（v0.1.73〜0.1.74・AccelTimerX限定）**: CoreMotion/Kalman由来の split/FINISH は
  直近GPS補正から1.6秒以内の時だけ許可する。GPSが途切れた状態で加速度積分だけが進むと、
  横加速度・端末揺れ・ドリフトで偽スプリット/偽FINISHになり得るため。GPS由来のスプリットは従来通り
  線形補間で処理する。**元の `AccelTimer` では無効**。
...
- 買い切り（非消費型 IAP `com.acceltimer.app.AccelTimerNeo.unlock`、¥800、StoreKit2）。
  ローカルテストは `AccelTimerNeo.storekit` をスキームが参照。`PaywallView` は「無制限解放」訴求。
...
`AccelTimerNeoTests` ターゲット（同期グループ方式、objectVersion 77）。
...
- **リプレイ基盤（`ReplayTests`・2026-06-22 / 人力テストループを断つ）**: 実走ログ(debug.csv)から切り出した
  サンプル列を `ReplaySample` 配列としてテストに埋め込み、判定ロジック（現状は `armedPhase`）へ通して
  **実機で再走行せずに過去の実データで回帰検証**する。`armedPhase` の「停車確認済み＋赤sAccでGPS確認中を
  出さずREADYにする」(12:36ログ)等を固定。**新しいログで問題を見つけたら、その区間を fixture に追加するだけで
  以降は数秒で永久回帰**（同じ現象で二度と走らなくてよい）。抽出方法とfixture形式は `ReplayTests.swift` 冒頭コメント参照。
  - 限界：positionSpeedKmh / 座標がログに無いため、停車確認(`updateArmedLaunch`)自体のフル・リプレイは不可。
    将来 debug.csv に positionSpeed/座標を足せば、停車確認・スプリット・FINISH までエンジン全体をリプレイ可能になる。
  - `AccelTimer` スキームの test は `-only-testing:AccelTimerNeoTests` で 42 件全緑。
...
`AccelTimerX` は現行アプリを壊さず根本修正・比較検証するための別アプリ。v0.1.76以降は
このリポジトリ内のターゲットではなく、ローカルの独立プロジェクト
`/Users/user01/dev/AccelTimerX/AccelTimerX.xcodeproj` で管理する。ソース本体は
`/Users/user01/dev/AccelTimerX/AccelTimerX/`。**以後の大胆な計測修正は独立プロジェクト側だけに入れ、
この `AccelTimer` リポジトリへ混ぜないこと。**

独立版の Bundle ID は `com.acceltimer.app.AccelTimerX`。後でアプリ名を変える場合は、独立プロジェクト側の
`Config/Version.xcconfig` を変更する。Keychain trial service と StoreKit product ID は Bundle ID 由来に
しているため、AccelTimerX は現行アプリと無料枠・課金状態を共有しない。
...
（リプレイ基盤 = `AccelTimerNeoTests/ReplayTests.swift`。§11参照。新ログの停車区間を ReplaySample 配列として
...
- **v0.1.73以降の `GPS_STALE_IGNORED`（AccelTimerX実験版限定）**: GPS fix自体はログに残すが、
  処理時刻から2秒超古いfixは READY/発進などのライブ状態遷移に使わない。このイベントが多い場合は、
  アプリロジックより先に iOS/環境側のGPS配信遅延が主因と見る。通常の `AccelTimer` では出ない。
