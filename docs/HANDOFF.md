...
最終更新時の状態: バージョン 0.1.74 系 / Swift 6 / SwiftUI / SwiftData / iOS 17+。
...
**このセッション（〜v0.1.74）の主な変更:** ①課金を透かしモデル→トライアル課金へ全面変更
...
同じ `AccelTimer/` ソースを共有するが、Bundle ID は `com.acceltimer.app.AccelTimerX`、
表示名/Product名は `Config/Version.xcconfig` の `ACCELTIMERX_APP_DISPLAY_NAME` で管理する。
後でアプリ名を変える場合はこの変数を変更する。通常の `AccelTimer` は
`ACCELTIMER_APP_DISPLAY_NAME` を使う。Keychain trial service と StoreKit product ID は
Bundle ID 由来にしているため、AccelTimerX は現行アプリと無料枠・課金状態を共有しない。
ローカル StoreKit 設定には `com.acceltimer.app.AccelTimer.unlock` と
`com.acceltimer.app.AccelTimerX.unlock` の両方を登録している。
計測ロジックの実験的変更も、必要に応じて `AppInfo.isAccelTimerX` で分岐し、元の
`AccelTimer` へ影響させない。v0.1.74時点ではGPS鮮度ゲートとMotion split鮮度ゲートが
AccelTimerX限定。
