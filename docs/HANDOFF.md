...
最終更新時の状態: バージョン 0.1.84 系 / Swift 6 / SwiftUI / SwiftData / iOS 17+。
...
**このセッション（〜v0.1.84）の主な変更:** ①課金を透かしモデル→トライアル課金へ全面変更
...
- **リプレイ基盤（`ReplayTests`・2026-06-22 / 人力テストループを断つ）**: 実走ログ(debug.csv)から切り出した
  ...
  **実機で再走行せずに過去の実データで回帰検証**する。`armedPhase` の「停車確認済み＋赤sAccでGPS確認中を
  ...
  - `AccelTimer` スキームの test は `-only-testing:AccelTimerTests` で 42 件全緑。
