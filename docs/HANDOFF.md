...
最終更新時の状態: バージョン 0.1.73 系 / Swift 6 / SwiftUI / SwiftData / iOS 17+。
...
**このセッション（〜v0.1.73）の主な変更:** ①課金を透かしモデル→トライアル課金へ全面変更
...
⑫GPSバッチ遅延対策として、処理時刻から2秒超古いGPS fixは READY/発進などライブ状態遷移に
使わず、CoreMotion由来スプリットも直近GPS補正が1.6秒以内の時だけ許可する（§2/§14, v0.1.73）。
...
- 発進トリガー条件: `confirmedStoppedWhileArmed && READY成立から0.5秒以上 &&
  ...
  **停車確認（GPS良好で速度≈0）が前提**。
- **GPS fix鮮度ゲート（v0.1.73）**: `CLLocation.timestamp` が処理時刻より2秒超古い
  ...
  `GPS_STALE_IGNORED` を出す。
- **Motion split のGPS鮮度ゲート（v0.1.73）**: CoreMotion/Kalman由来の split/FINISH は
...
- **v0.1.73以降の `GPS_STALE_IGNORED`**: GPS fix自体はログに残すが、処理時刻から2秒超古いfixは
  READY/発進などのライブ状態遷移に使わない。このイベントが多い場合は、アプリロジックより先に
  iOS/環境側のGPS配信遅延が主因と見る。
