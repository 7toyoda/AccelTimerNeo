...
最終更新時の状態: バージョン 0.1.75 系 / Swift 6 / SwiftUI / SwiftData / iOS 17+。
...
**このセッション（〜v0.1.75）の主な変更:** ①課金を透かしモデル→トライアル課金へ全面変更
...
⑪別アプリとして実機比較するため `AccelTimerX` ターゲット/スキームを追加（§13, v0.1.72）。
...
⑬`AccelTimerX` を `AccelTimerX/` フォルダへ物理分離し、元の `AccelTimer/` ソースを共有しない構成へ変更
（§13, v0.1.75）。以後の大胆な計測修正は原則 `AccelTimerX/` 側だけに入れる。
...
- **リプレイ基盤（`ReplayTests`・2026-06-22 / 人力テストループを断つ）**: 実走ログ(debug.csv)から切り出した
  ...
  **実機で再走行せずに過去の実データで回帰検証**する。`armedPhase` の「停車確認済み＋赤sAccでGPS確認中を
  ...
  - ⚠️ `AccelTimer` スキームの test を**全ターゲットで**走らせると、別アプリ比較用 `AccelTimerX` のテスト起動で
    クラッシュする（早期exit）。**`-only-testing:AccelTimerTests` で当該ターゲットに絞れば 42 件全緑**。
...
`AccelTimerX` は現行アプリを壊さず根本修正・比較検証するための別アプリターゲット。
v0.1.75以降は `AccelTimerX/` フォルダに Swift/UI/権限/StoreKit/String Catalog を物理コピーし、
ターゲットの同期ルートも `AccelTimer/` から `AccelTimerX/` へ切り替え済み。**以後の大胆な
計測修正は原則 `AccelTimerX/` 側だけに入れ、元の `AccelTimer/` へ混ぜないこと。**

Bundle ID は `com.acceltimer.app.AccelTimerX`、表示名/Product名は
`Config/Version.xcconfig` の `ACCELTIMERX_APP_DISPLAY_NAME` で管理する。後でアプリ名を変える場合は
この変数を変更する。通常の `AccelTimer` は `ACCELTIMER_APP_DISPLAY_NAME` を使う。
Keychain trial service と StoreKit product ID は Bundle ID 由来にしているため、AccelTimerX は
現行アプリと無料枠・課金状態を共有しない。ローカル StoreKit 設定は `AccelTimerX/AccelTimer.storekit`
を `AccelTimerX` scheme が参照する。
...
- **v0.1.73以降の `GPS_STALE_IGNORED`（AccelTimerX限定）**: GPS fix自体はログに残すが、
