...
- **バージョンを上げる**：コード変更を伴う修正はコミット前に `MARKETING_VERSION`
  （pbxproj 内 4 箇所＝アプリ＆テストターゲットの Debug/Release）のパッチを +1
  （例 0.1.30 → 0.1.31）。正式リリース（1.0.0 等）への昇格は要相談。ドキュメント
  のみの変更は据え置き。
- 変更後は下記コマンドで **ビルド成功・テスト通過** を確認してからコミットする。
- 既存の設計判断（docs/HANDOFF.md）を壊さないこと。特に「減速リセットは意図的に
  無効」「手持ち計測を許可し参考値扱い」「課金は履歴5件まで無料のハイブリッド」は仕様。
